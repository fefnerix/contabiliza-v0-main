...
// Cache para almacenar las configuraciones de plan
let planConfigCache: any = null;

/**
 * Busca las configuraciones de plan del Supabase
 */
export async function getPlanConfig() {
  if (planConfigCache) return planConfigCache;
  
  try {
    console.log('[getPlanConfig] Iniciando búsqueda de configuraciones...');
    
    // Buscar price IDs de las configuraciones del Stripe
    const { data: priceData, error: priceError } = await supabase.functions.invoke('get-stripe-prices');
    console.log('[getPlanConfig] Respuesta get-stripe-prices:', { priceData, priceError });
    
    if (priceError || !priceData?.success) {
      throw new Error(priceError?.message || priceData?.error || 'Failed to fetch price IDs');
    }

    // Buscar configuraciones públicas de pricing
    ...
    if (adminData?.success && adminData?.settings?.pricing) {
      console.log('[getPlanConfig] Procesando configuraciones de pricing:', adminData.settings.pricing);
      
      // Extraer valores de la estructura de pricing
      ...
      planConfigCache = {
        prices: {
          monthly: {
            ...
            displayPrice: settings.plan_price_monthly || 'US$ -',
          },
          annual: {
            ...
            displayPrice: settings.plan_price_annual || 'US$ -',
            displayOriginalPrice: settings.plan_price_monthly_equivalent || 'US$ -',
    ...
    console.error('Error al cargar configuraciones de plan:', err);
    return null;
  }
}
