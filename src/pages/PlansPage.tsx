...
const PlansPage = () => {
  ...
  useEffect(() => {
    if (success === 'true') {
      ...
      // Mostrar carga mientras busca las configuraciones
  if (configLoading) {
    return (
  ...
  const plans = [
    {
      ...
      price: config?.prices.monthly.displayPrice || formatMoney(29.90, { currency: 'USD', onlySymbol: true }),
      period: "/mes",
    ...
    {
      name: t('plans.annual'),
      price: config?.prices.annual.displayPrice || formatMoney(177.00, { currency: 'USD', onlySymbol: true }),
      period: "/año",
      priceId: config?.prices.annual.priceId,
      originalPrice: config?.prices.annual.displayOriginalPrice || formatMoney(238.80, { currency: 'USD', onlySymbol: true }),
      savings: config?.prices.annual.displaySavings || 'Ahorra 25%',
