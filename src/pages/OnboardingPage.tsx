        ...
        {step === 2 && (
          <Card className="p-4 sm:p-6 space-y-6 sm:space-y-8 border-border/70 shadow-sm [&_section]:rounded-xl [&_section]:border [&_section]:border-border/70 [&_section]:bg-muted/20 [&_section]:p-4 sm:[&_section]:p-5 [&_section>h2]:mb-1 [&_section>h2]:text-base sm:[&_section>h2]:text-lg [&_section>h2]:font-semibold [&_section>h2]:tracking-tight [&_section>div]:space-y-1.5 [&_label]:text-sm [&_label]:font-medium [&_input]:h-11 [&_textarea]:min-h-[104px] [&_[role=combobox]]:h-11">
            ...
            <section className="space-y-4">
              ...
              <div className="space-y-2"><Label>1.5 ¿En qué moneda quieres trabajar tu plan?</Label><Select value={form.currency} onValueChange={(v) => setField("currency", v)}><SelectTrigger><SelectValue placeholder="Selecciona moneda" /></SelectTrigger><SelectContent>{CURRENCY_OPTIONS.map((x) => <SelectItem key={x} value={x}>{x}</SelectItem>)}</SelectContent></Select>{form.currency === "Otra (especifica cuál)" && <Input placeholder="Especifica moneda" value={form.currencyOther} onChange={(e) => setField("currencyOther", e.target.value)} />}</div>
