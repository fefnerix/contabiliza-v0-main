...
const BrandingConfigManager: React.FC = () => {
  ...
  const loadBrandingConfig = async () => {
    try {
      ...
      if (error) {
        console.error('Error al cargar configuraciones de branding:', error);
        return;
      }
      ...
      console.error('Error al cargar configuraciones de branding:', err);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('not found') || error.message.includes('does not exist')) {
          throw new Error('Bucket de storage no encontrado. Ejecuta el deployment de Supabase para crear el bucket "uploads".');
        }
  ...
  const handleSave = async () => {
    try {
      ...
      if (data?.success) {
        ...
        toast({
          title: "¡Configuración de marca guardada!",
          description: "Logo y favicon fueron actualizados con éxito. Actualizando todas las pestañas...",
        });
        
        console.log('✅ Branding guardado con éxito - cache invalidado para todas las pestañas');
        
        // Recargar configuraciones después de guardar
        await loadBrandingConfig();
      }
    } catch (error: any) {
      console.error('Error al guardar configuraciones de branding:', error);
      toast({
        title: "Error al guardar",
        description: error.message || 'No fue posible guardar la configuración de marca.',
        variant: "destructive",
      });
    } finally {
      setIsUpdating(false);
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-center justify-center">
            ...
            <span>Cargando configuración de marca...</span>
          </div>
        </CardContent>
      </Card>
    );
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-gray-600">
            No tienes permisos para acceder a la configuración de marca.
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ex: Gestiona tus finanzas con Contabiliza"
              disabled={isUpdating}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="border rounded-lg p-4 bg-gray-50">
                <img 
                  ...
                  alt={formData.logoAltText || "Logo de la empresa"}
          ...
          <div className="space-y-2">
            <Label htmlFor="logoFile">Subir Nueva Logo (PNG, JPG, SVG)</Label>
            <Input
              id="logoFile"
              type="file"
            ...
            {logoFile && (
              <p className="text-sm text-green-600">
                Nueva logo seleccionada: {logoFile.name}
              </p>
            )}
          </div>

          <div className="space-y-2">
            <Label htmlFor="logoAltText">Texto Alternativo de la Logo</Label>
            <Input
              id="logoAltText"
              value={formData.logoAltText}
              onChange={(e) => setFormData(prev => ({ ...prev, logoAltText: e.target.value }))}
              placeholder="Logo de la empresa Contabiliza"
              disabled={isUpdating}
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="landingTheme">Tema Forzado de la Página de Inicio</Label>
            <Select 
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="system">Sistema (preferencia del usuario)</SelectItem>
                <SelectItem value="light">Claro (siempre)</SelectItem>
                <SelectItem value="dark">Oscuro (siempre)</SelectItem>
              </SelectContent>
            </Select>
            <p className="text-xs text-muted-foreground">
              Define el tema que será siempre usado en la página inicial, independiente de la configuración del usuario.
            </p>
          </div>
        </div>
            ...
            <div className="space-y-2">
              ...
              <div className="border rounded-lg p-4 bg-gray-50 flex items-center gap-2">
                <img 
                  ...
                  className="w-4 h-4"
                />
                <span className="text-sm text-gray-600">Favicon del sitio</span>
              </div>
            </div>
          )}
          
          <div className="space-y-2">
            <Label htmlFor="faviconFile">Subir Nuevo Favicon (PNG, JPG)</Label>
            <Input
              id="faviconFile"
              type="file"
            ...
            {faviconFile && (
              <p className="text-sm text-green-600">
                Nuevo favicon seleccionado: {faviconFile.name}
              </p>
            )}
            <p className="text-xs text-gray-500">
              Recomendado: imagen cuadrada, mínimo 32x32px
            </p>
          </div>
        </div>
