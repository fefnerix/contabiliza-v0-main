              ...
              <Card key={transaction.id} className="p-4">
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span>{new Date(transaction.date).toLocaleDateString('es-419')}</span>
                  </div>
                  <div className="font-medium">{translateCategoryName(transaction.category, transaction.type, 'es-419')}</div>
              ...
              <tbody>
                {transactions.map((transaction) => (
                  <tr key={transaction.id} className="border-b hover:bg-muted">
                    <td className="py-2 px-4">{new Date(transaction.date).toLocaleDateString('es-419')}</td>
                    <td className="py-2 px-4">
                      {transaction.type === 'income' ? t('common.income') : t('common.expense')}
                    </td>
                    <td className="py-2 px-4">{translateCategoryName(transaction.category, transaction.type, 'es-419')}</td>
